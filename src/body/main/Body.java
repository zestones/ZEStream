...
public class Body implements IBody {
		...
		for (String t : Bibliotheque.seriesTitle) {
			if (t.equals(fileName)) {
				...
				if (currentTab.equals(SERIES_TAB)) {
					Image bookmarks = new Image(new Position(cardPos.getX() + CARD_WIDTH, cardPos.getY()),
							BOOKMARK_ICON, new Dimension(20, 78));
	...
	private void fillBody(ArrayList<String> coverPathArray) {
		...
		for (String path : coverPathArray) {
			...
			if (cardPos.getY() >= sp.posView.getY()
					&& cardPos.getY() <= sp.posView.getY() + FRAME_HEIGHT - MENU_HEIGHT) {
				Button series = new Button(cardPos, path, fileName, DIM_CARD, true);

				Color c = getCardColor(cardPos, fileName);

				txt = new Button(
						new Position(cardPos.getX() - 20,
								cardPos.getY() + (int) series.getDimension().getHeight() + 10),
						fileName, 18, c, DARK_THEME, Font.PLAIN);
				...
				if (currentTab.equals(LIBRARY_TAB)) {
					Button delete = new Button(new Position(cardPos.getX() + CARD_WIDTH, cardPos.getY()), DELETE_ICON,
							fileName, new Dimension(25, 25), false);
				...
				if (lastDisplayedPos.getX() + CARD_WIDTH + PADDING_CARDS_SIDE >= FRAME_WIDTH
						&& index < coverPathArray.size() - 1) {
		...
		container.setPreferredSize(
				new Dimension(CONTAINER_WIDTH, cardPos.getY() + CARD_HEIGHT + txt.getHeight() + PADDING_CARDS_TOP));
	}
		...
		for (String t : Bibliotheque.seriesTitle) {
			if (t.equals(parent)) {
				...
				if (OsUtils.isWindows()) {
					...
					String parentpath = parentPathName.replace(IGlobal.SEPARATOR,
							IGlobal.SEPARATOR + IGlobal.SEPARATOR);

					String[] separated = titlePath.split(parentpath);
	...
	private void fillBody(String path) {
		...
		for (String file : filesArray) {
			...
			if (!FileSearch.getFileName(file).equalsIgnoreCase(COVER_FOLDER_NAME)) {
				...
				Button series = new Button(cardPos, parentPathName + IGlobal.SEPARATOR + COVER_FOLDER_NAME
						+ IGlobal.SEPARATOR + file + "." + extension, FileSearch.getFileName(file), DIM_CARD, true);
				...
				txt = new Button(
						new Position(cardPos.getX() - 20,
								cardPos.getY() + (int) series.getDimension().getHeight() + 10),
						file, 18, c, DARK_THEME, Font.PLAIN);

				if (txt.getTextDimension().getWidth() > DIM_CARD.getWidth())
					txt = adjustTextButton(file, series, cardPos);

				txt.setImagePath(folderPath + IGlobal.SEPARATOR + COVER_FOLDER_NAME + IGlobal.SEPARATOR + file + "."
						+ extension);
	...
	private static Button adjustTextButton(String fileName, Button series, Position cardPos) {
		...
		Button txt = new Button(
				new Position(cardPos.getX() - 20, cardPos.getY() + (int) series.getDimension().getHeight() + 10),
				separated[0], 18, Color.white, DARK_THEME, Font.PLAIN);
		...
		while (d.getWidth() < DIM_CARD.getWidth()) {
			...
			if (new Button(
					...
					c,
					DARK_THEME,
					Font.PLAIN).getTextDimension().getWidth() >= DIM_CARD.getWidth())
				break;
