...
public class EpisodeEvent implements IMenu, IEpisode {
	...
	private void updateBookmarkSeries(int index) {
		boolean success = Bibliotheque.updateBiblioPathFolders(Body.parentPathName,
				episodeButtonArray.get(index).getText(), Episode.currentFolderPath);
						...
						if (e.getSource() == episodeButtonArray.get(j)) {
							try {
								Desktop.getDesktop().open(new File(
										Episode.currentFolderPath + SEPARATOR + episodeButtonArray.get(j).getText()));
				...
				public void mouseMoved(MouseEvent e) {
					for (int j = 0; j < episodeButtonArray.size(); j++) {
						if (btn.isHoverActive())
							episodeButtonArray.get(j).unsetHoverButton();
	...
	public static void removeBookmarkButton() {
		if (bookmarkEpbtn != null)
			container.remove(bookmarkEpbtn);
		if (delBookmarksEpBtn != null)
			container.remove(delBookmarksEpBtn);
