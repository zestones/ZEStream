				...
				public void actionPerformed(ActionEvent e) {
					...
					for (int j = 0; j < deleteButtonArray.size(); j++) {
						if (e.getSource() == deleteButtonArray.get(j)) {
							...
							PopUp confirm = new PopUp("Etes-vous sûres de vouloir supprimer le lien ?",
									"Selectioner une option", JOptionPane.YES_NO_CANCEL_OPTION,
	...
	private void handleCoverFolderEvent(Button btn) {
		...
		btn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Position pos = new Position(Setting.folderCoverBtn.getPosition().getX(),
						Setting.folderCoverBtn.getPosition().getY());
