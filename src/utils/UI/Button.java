...
public class Button extends JButton implements MouseMotionListener, IGlobal {
	...
	public Color upadteHoverColor(Color c, int HOVER_DIFF) {
		...
		return new Color(
				Math.abs(c.getRed() + HOVER_DIFF) < 255 ? Math.abs(c.getRed() + HOVER_DIFF) : c.getRed(),
				Math.abs(c.getGreen() + HOVER_DIFF) < 255 ? Math.abs(c.getGreen() + HOVER_DIFF) : c.getGreen(),
				Math.abs(c.getBlue() + HOVER_DIFF) < 255 ? Math.abs(c.getBlue() + HOVER_DIFF) : c.getBlue());
	}
